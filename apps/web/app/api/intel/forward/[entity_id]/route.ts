...
export async function GET(
  ...
  try {
    ...
    if (!Uuid.safeParse(entity_id).success)
    ...
    if (fixturesEnabled()) {
      ...
      const docs = new Set(
        corpus.docs
          .filter((doc) => doc.corpus_id === entity.corpus_id)
          .map((doc) => doc.id),
      );
      const canonical =
        corpus.nodes.find((node) => node.id === entity.canonical_node_id)
          ?.entity_id ?? entity_id;
      return jsonOf(ForwardResponse, {
        ...forwardGraph(
    ...
    return jsonOf(ForwardResponse, {
      ...forwardGraph(
        graph.nodes,
        graph.edges,
        (await canonicalEntity(entity_id)) ?? entity_id,
      ),
      entity_id,
    });
  } catch (error) {
