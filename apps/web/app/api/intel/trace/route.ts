...
export async function POST(req: Request) {
  ...
  if (fixturesEnabled()) {
    ...
    return jsonOf(TraceResponse, authored ?? traceSelection(corpus.nodes.filter((node) => docs.has(node.doc_id)), corpus.edges, input, corpus.anchors));
  }
