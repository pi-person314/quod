...
export async function GET(req: Request) {
  try {
    ...
    if (!Uuid.safeParse(corpusId).success)
      return Response.json({ error: "Invalid corpus_id" }, { status: 400 });
    ...
    const result = await db().query(
      `SELECT stage, count(*)::int AS calls,
    ...
    return Response.json({
      stages: result.rows,
      total_usd: result.rows.reduce((sum, row) => sum + row.cost_usd, 0),
      measured: result.rowCount! > 0,
    });
  } catch (error) {
