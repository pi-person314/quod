...
export async function POST(req: Request) {
  ...
  if (fixturesEnabled()) {
    ...
    if (!corpus.docs.some((doc) => doc.id === body.doc_id)) return notFound("document");
    const anchors = new Set(corpus.anchors.filter((anchor) => anchor.doc_id === body.doc_id).map((anchor) => anchor.id));
    return jsonOf(BakeResponse, { cards_done: new Set(corpus.cards.filter((card) => anchors.has(card.anchor_id)).map((card) => card.anchor_id)).size });
  }
