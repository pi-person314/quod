...
export async function POST(req: Request) {
  try {
    ...
    if (fixturesEnabled()) {
      ...
      const docs = new Set(
        corpus.docs
          .filter((doc) => doc.corpus_id === body.corpus_id)
          .map((doc) => doc.id),
      );
      ...
      return jsonOf(ResolveResponse, {
        decisions: nodes
          .filter((node) => node.entity_id && entities.has(node.entity_id))
          .map((node) => ({
            node_id: node.id,
    ...
    try {
      ...
      console.error("Resolution failed", {
        stage: error instanceof ResolutionStageError ? error.stage : "setup",
        type: cause instanceof Error ? cause.name : "UnknownError",
        code: typeof (cause as { code?: unknown })?.code === "string" ? (cause as { code: string }).code : undefined,
      ...
      const known = [
        ...
        "Missing or unexpected batch adjudications",
        "Unexpected or duplicate adjudication",
        "Model response incomplete, refused, or empty",
        "Corpus changed during resolution; retry required",
        "Candidate is outside the corpus or is the source node",
        "Resolution pair exceeds the context bound",
        "Search numeric mapping is incompatible; migrate the index before ingesting",
      ];
      const message =
        error instanceof ResolutionStageError || (error instanceof Error && known.includes(error.message))
          ? error.message
